...
describe('createBounty', () => {
  ...
  it('creates invoice from LNbits when apiKey provided without url', async () => {
    ...
    // lnbitsUrl is falsy (''), but apiKey is provided. Explicit 'custodial'
    // since NWC is now the default and only the custodial branch mints invoices.
    const result = await createBounty(5000, '', 'my-api-key', mockTest, 'creator', undefined, 'custodial');
  ...
  it('defaults to nwc and forwards fundingMode=nwc in the POST body', async () => {
    jest.spyOn(global, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ id: 'bounty-uuid' }),
    } as any);

    // No explicit fundingMode → defaults to 'nwc' (custodial is disabled).
    await createBounty(5000, undefined, undefined, mockTest, 'creator-pub');
    const body = JSON.parse((global.fetch as jest.Mock).mock.calls[0][1].body);
    expect(body.fundingMode).toBe('nwc');
  });

  it('omits fundingMode from the body when explicitly custodial (backward-compat)', async () => {
    jest.spyOn(global, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ id: 'bounty-uuid' }),
    } as any);

    // Explicit custodial is still serialized by omission so older backends
    // that don't know the field keep working.
    await createBounty(5000, undefined, undefined, mockTest, 'creator-pub', undefined, 'custodial');
    const body = JSON.parse((global.fetch as jest.Mock).mock.calls[0][1].body);
...
describe('approveClaim', () => {
  it('returns updated bounty on success', async () => {
    ...
    const result = await approveClaim('bounty-1', claimId);
  ...
  it('surfaces the backend `error` field (NWC 502) instead of a generic status', async () => {
    // The NWC payout failure returns { error: <reason> } with a 502. The toast
    // should show the real reason, not just "Approval failed: 502".
    jest.spyOn(global, 'fetch').mockResolvedValue({
      ok: false,
      status: 502,
      json: async () => ({ error: 'NWC payout failed: reply timeout' }),
    } as any);

    const result = await approveClaim('bounty-1', 'approver-pub');
    expect(result).toBeNull();
    expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
      expect.stringContaining('NWC payout failed: reply timeout')
    );
  });
...
describe('getLnurlLimits', () => {
  it('returns the sendable bounds on success and POSTs the trimmed lnurl', async () => {
    const fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ minSendable: 1000, maxSendable: 100_000_000 }),
    } as any);

    const result = await getLnurlLimits('  alice@primal.net  ');
    expect(result).toEqual({ minSendable: 1000, maxSendable: 100_000_000 });
    expect(fetchSpy).toHaveBeenCalledWith(
      'http://localhost:3000/lnurl/limits',
      expect.objectContaining({ method: 'POST' })
    );
    const body = JSON.parse(fetchSpy.mock.calls[0][1]!.body as string);
    expect(body.lnurl).toBe('alice@primal.net');
  });

  it('returns null (fail open) on a non-OK response', async () => {
    jest.spyOn(global, 'fetch').mockResolvedValue({ ok: false, status: 400 } as any);
    expect(await getLnurlLimits('lnurl1bad')).toBeNull();
  });

  it('returns null (fail open) when the body is missing numeric bounds', async () => {
    jest.spyOn(global, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({}),
    } as any);
    expect(await getLnurlLimits('alice@primal.net')).toBeNull();
  });
