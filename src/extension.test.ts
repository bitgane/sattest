...
jest.mock('./api/nwc.api', () => ({
  ...
  confirmBackendForNwc: jest.fn().mockResolvedValue(true),
}));
...
describe('extension', () => {
  ...
  describe('connectWallet command', () => {
    ...
    it('aborts without sending the URI when the backend is not confirmed', async () => {
      const { getNostrUserPubkey } = require('./state');
      (getNostrUserPubkey as jest.Mock).mockResolvedValue('npub-pub');
      (vscode.window.showInputBox as jest.Mock).mockResolvedValue('nostr+walletconnect://abc');
      const { setNwcUri, confirmBackendForNwc } = require('./api/nwc.api');
      (confirmBackendForNwc as jest.Mock).mockClear().mockResolvedValue(false);

      const handler = await captureHandler('sattest.connectWallet');
      await handler();

      expect(confirmBackendForNwc).toHaveBeenCalled();
      expect(setNwcUri).not.toHaveBeenCalled();
      expect(vscode.window.showWarningMessage).toHaveBeenCalledWith(
        expect.stringContaining('backend not confirmed')
      );
      // Restore for subsequent tests.
      (confirmBackendForNwc as jest.Mock).mockResolvedValue(true);
    });
